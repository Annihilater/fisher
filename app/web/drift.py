    ...
    if request.method == "POST":
        if form.validate():
            ...
            wish = Wish.query.filter_by(
                uid=current_user.id, isbn=current_gift.isbn
            ).first()
            if not wish:
                with db.auto_commit():
                    wish = Wish()
...
def reject_drift(did):
    with db.auto_commit():
        drift = Drift.query.filter(
            Gift.uid == current_user.id, id == did
        ).first_or_404()
...
def redraw_drift(did):
    with db.auto_commit():
        ...
        drift = Drift.query.filter_by(
            requester_id=current_user.id, id=did
        ).first_or_404()
...
def mailed_drift(did):
    with db.auto_commit():
        ...
        wish = Wish.query.filter_by(
            uid=drift.requester_id, isbn=drift.isbn, launched=False
        ).first_or_404()
        wish.launched = True
